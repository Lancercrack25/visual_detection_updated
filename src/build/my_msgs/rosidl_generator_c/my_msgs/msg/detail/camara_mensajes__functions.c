// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from my_msgs:msg/CamaraMensajes.idl
// generated code does not contain a copyright notice
#include "my_msgs/msg/detail/camara_mensajes__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `camera_id`
// Member `image_format`
#include "rosidl_runtime_c/string_functions.h"
// Member `image`
#include "sensor_msgs/msg/detail/compressed_image__functions.h"

bool
my_msgs__msg__CamaraMensajes__init(my_msgs__msg__CamaraMensajes * msg)
{
  if (!msg) {
    return false;
  }
  // camera_id
  if (!rosidl_runtime_c__String__init(&msg->camera_id)) {
    my_msgs__msg__CamaraMensajes__fini(msg);
    return false;
  }
  // image_format
  if (!rosidl_runtime_c__String__init(&msg->image_format)) {
    my_msgs__msg__CamaraMensajes__fini(msg);
    return false;
  }
  // image
  if (!sensor_msgs__msg__CompressedImage__init(&msg->image)) {
    my_msgs__msg__CamaraMensajes__fini(msg);
    return false;
  }
  return true;
}

void
my_msgs__msg__CamaraMensajes__fini(my_msgs__msg__CamaraMensajes * msg)
{
  if (!msg) {
    return;
  }
  // camera_id
  rosidl_runtime_c__String__fini(&msg->camera_id);
  // image_format
  rosidl_runtime_c__String__fini(&msg->image_format);
  // image
  sensor_msgs__msg__CompressedImage__fini(&msg->image);
}

bool
my_msgs__msg__CamaraMensajes__are_equal(const my_msgs__msg__CamaraMensajes * lhs, const my_msgs__msg__CamaraMensajes * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // camera_id
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->camera_id), &(rhs->camera_id)))
  {
    return false;
  }
  // image_format
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->image_format), &(rhs->image_format)))
  {
    return false;
  }
  // image
  if (!sensor_msgs__msg__CompressedImage__are_equal(
      &(lhs->image), &(rhs->image)))
  {
    return false;
  }
  return true;
}

bool
my_msgs__msg__CamaraMensajes__copy(
  const my_msgs__msg__CamaraMensajes * input,
  my_msgs__msg__CamaraMensajes * output)
{
  if (!input || !output) {
    return false;
  }
  // camera_id
  if (!rosidl_runtime_c__String__copy(
      &(input->camera_id), &(output->camera_id)))
  {
    return false;
  }
  // image_format
  if (!rosidl_runtime_c__String__copy(
      &(input->image_format), &(output->image_format)))
  {
    return false;
  }
  // image
  if (!sensor_msgs__msg__CompressedImage__copy(
      &(input->image), &(output->image)))
  {
    return false;
  }
  return true;
}

my_msgs__msg__CamaraMensajes *
my_msgs__msg__CamaraMensajes__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_msgs__msg__CamaraMensajes * msg = (my_msgs__msg__CamaraMensajes *)allocator.allocate(sizeof(my_msgs__msg__CamaraMensajes), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(my_msgs__msg__CamaraMensajes));
  bool success = my_msgs__msg__CamaraMensajes__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
my_msgs__msg__CamaraMensajes__destroy(my_msgs__msg__CamaraMensajes * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    my_msgs__msg__CamaraMensajes__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
my_msgs__msg__CamaraMensajes__Sequence__init(my_msgs__msg__CamaraMensajes__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_msgs__msg__CamaraMensajes * data = NULL;

  if (size) {
    data = (my_msgs__msg__CamaraMensajes *)allocator.zero_allocate(size, sizeof(my_msgs__msg__CamaraMensajes), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = my_msgs__msg__CamaraMensajes__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        my_msgs__msg__CamaraMensajes__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
my_msgs__msg__CamaraMensajes__Sequence__fini(my_msgs__msg__CamaraMensajes__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      my_msgs__msg__CamaraMensajes__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

my_msgs__msg__CamaraMensajes__Sequence *
my_msgs__msg__CamaraMensajes__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_msgs__msg__CamaraMensajes__Sequence * array = (my_msgs__msg__CamaraMensajes__Sequence *)allocator.allocate(sizeof(my_msgs__msg__CamaraMensajes__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = my_msgs__msg__CamaraMensajes__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
my_msgs__msg__CamaraMensajes__Sequence__destroy(my_msgs__msg__CamaraMensajes__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    my_msgs__msg__CamaraMensajes__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
my_msgs__msg__CamaraMensajes__Sequence__are_equal(const my_msgs__msg__CamaraMensajes__Sequence * lhs, const my_msgs__msg__CamaraMensajes__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!my_msgs__msg__CamaraMensajes__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
my_msgs__msg__CamaraMensajes__Sequence__copy(
  const my_msgs__msg__CamaraMensajes__Sequence * input,
  my_msgs__msg__CamaraMensajes__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(my_msgs__msg__CamaraMensajes);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    my_msgs__msg__CamaraMensajes * data =
      (my_msgs__msg__CamaraMensajes *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_msgs__msg__CamaraMensajes__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          my_msgs__msg__CamaraMensajes__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!my_msgs__msg__CamaraMensajes__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
